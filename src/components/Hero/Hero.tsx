...
const Hero = ({ blok, className }: HeroPropType) => {
  return (
    <div
      className={clsx(styles.Hero, className, `min-h-screen flex justify-center p-10`)}
        ...
        <div className='mb-[100px]'>
          ...
          >
            <div className='w-full h-full relative overflow-hidden rounded-full'>
              <Image className=' w-[121px] h-[121px] ' />
            </div>
          </div>
        </div>
        <h1 className='text-white mb-0 text-8xl'>{blok.title}</h1>
      </div>
    </div>
  );
};
